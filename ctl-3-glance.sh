...
glance_config_api () {
	...
	ops_add $glanceapifile database connection mysql+pymysql://glance:$GLANCE_DBPASS@$CTL_EXT_IP/glance
	ops_add $glanceapifile keystone_authtoken auth_url http://$CTL_EXT_IP:5000
    ops_add $glanceapifile keystone_authtoken www_authenticate_uri http://$CTL_EXT_IP:5000
	ops_add $glanceapifile keystone_authtoken memcached_servers $CTL_EXT_IP:11211	  
