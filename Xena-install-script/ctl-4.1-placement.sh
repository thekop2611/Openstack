...
placement_config () {
	...
	crudini --set $placementfile placement_database connection mysql+pymysql://placement:$PLACEMENT_DBPASS@$CTL_EXT_IP/placement

	crudini --set $placementfile api auth_strategy keystone

	crudini --set $placementfile keystone_authtoken auth_url http://$CTL_EXT_IP:5000/v3
	crudini --set $placementfile keystone_authtoken memcached_servers $CTL_EXT_IP:11211
